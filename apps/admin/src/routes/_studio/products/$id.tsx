...
function skuToEntry(sku: {
  ...
  return {
    id: uid(),
    attrs: Object.entries(sku.attrs).map(([key, value]) => ({
      id: uid(),
      key,
      value,
    })),
...
function EditProductPage() {
  ...
  useEffect(() => {
    if (product.coverUrl) setCoverPreview(product.coverUrl);
    if (product.skus) {
      setSkus(product.skus.map(skuToEntry));
    }
  }, [product]);
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        <Card padding={false} className="p-4">
              ...
              <div className="flex items-center gap-3 rounded-lg border border-border bg-surface p-3">
                ...
                <div className="min-w-0 flex-1">
                  <p className="truncate text-sm text-text">
                    {coverInputRef.current?.files?.[0]?.name ?? "当前封面"}
                  </p>
          ...
          <div className="mt-3 grid grid-cols-1 gap-3 md:grid-cols-2">
            <Select label="状态" name="status" defaultValue={product.status}>
